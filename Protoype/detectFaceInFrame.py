        ...
        if center_x > frame_width / 2 and center_y < frame_height / 2:
            if(self.quadrant_start_time is None):
                ...
                if elapsed_time >= self.quadrant_duration_threshold:
                    print("Face detected in upper right quadrant")
        else:
            self.quadrant_start_time = None
...
try:
    ...
    while cap.isOpened():
        ...
        if face_center is not None:
            ...
            #Check if the face center is in the upper right quadrant
            face_detector.check_upper_right_quadrant(center_x, center_y, image.shape[1], image.shape[0])
